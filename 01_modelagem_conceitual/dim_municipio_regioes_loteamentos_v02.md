...
**Versão anterior:** v01 (03/03/2026)  
**Atualizações v02:**
- Eliminação do termo "território" — ver `glossario_conceitual_v01.md`
- `DIM_REGIAO` renomeada para `DIM_REGIAO_PLANEJAMENTO` — ancorada no Plano Diretor 2017–2020
- `REL_UNIDADE_LOTEAMENTO` renomeada para `REL_LOTEAMENTO_NUCLEO` — com campo `setor`
- `cod_loteamento` resolvido: adotar `codbairro` como código oficial municipal
- Conceito de Núcleo multi-setor incorporado — ver `dim_nucleo_v01.md`
- 141 loteamentos populados com dados oficiais da fonte `loteamentosregiao.xlsx`
...
> ⚠️ 11 loteamentos em fronteira entre duas RPs — campo `fronteira = S`.
...
> Substitui `REL_UNIDADE_LOTEAMENTO` do v01.
...
> **Controle temporal preservado do v01:** a cada reorganização de abrangência,
> os registros anteriores recebem `data_fim` e novos registros são criados.
...
| Análise por tipo | Vulnerabilidade em vilas × jardins × conjuntos habitacionais |
| Loteamentos novos | 15 aprovados entre 2021–2025 — monitorar cobertura |
| Ficha analítica do loteamento | População × PCDs × renda média × serviços que atendem |
...
> Validam empiricamente o indicador de cobertura.

| loteamento | codbairro | situação |
|------------|-----------|---------|
| Jardim São Bento | 341 | CadÚnico levado à UBS — distância do CRAS |
| Taquara Branca | ⚠️ não localizado | CadÚnico levado à UBS — distância do CRAS |

> ⚠️ "Taquara Branca" não consta na base oficial de loteamentos.
...
| # | pendência | prioridade | status v01 |
|---|-----------|------------|------------|
...
| P06 | Investigar "Taquara Branca" — nome popular sem registro cartorial | Alta | Nova |
| P07 | Adicionar CEP e coordenadas geográficas a cada loteamento | Alta | Nova |
| P08 | Construir TAB_NORMALIZA_LOCALIDADE para cruzamento com CadÚnico | Alta | Nova |
| P09 | Mapear 11 loteamentos de fronteira — confirmar RP correta | Média | Nova |
| P10 | Verificar cobertura dos 15 loteamentos aprovados 2021–2025 | Alta | Nova |
...
| v02 | 05/03/2026 | Termo "território" eliminado. `DIM_REGIAO` → `DIM_REGIAO_PLANEJAMENTO`. `REL_UNIDADE_LOTEAMENTO` → `REL_LOTEAMENTO_NUCLEO` com campo `setor`. `codbairro` adotado como chave oficial (P04 resolvida). Plano Diretor incorporado (P05 resolvida). 141 loteamentos populados. 11 fronteiras identificadas. |

---

*Documento de registro interno — Projeto de Inteligência Analítica — Hortolândia, março de 2026*
