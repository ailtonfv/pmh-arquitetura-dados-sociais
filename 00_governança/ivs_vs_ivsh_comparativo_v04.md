...
Infraestrutura Urbana	3	33%	~15–20%	Saneamento e coleta de lixo com alta cobertura em Hortolândia — baixo poder discriminatório entre territórios. Mantida IU_03 (mobilidade) por relevância estratégica.
Capital Humano	8	33%	~40–45%	Dimensão mais relevante localmente. Reprodução intergeracional da pobreza é o desafio central. 5 variáveis disponíveis imediatamente no CadÚnico.
...
TOTAL	16	100%	100%	—
> ⚠️ Pesos IVS-H são hipótese inicial. Calibração definitiva seguirá as diretrizes da
> OCDE (*Handbook on Constructing Composite Indicators*, 2008).
---
> **Nota territorial:**
> No Atlas Social de Hortolândia, o IVS-H é calculado na escala mínima de **loteamento**
> e posteriormente agregado para **Núcleo (área de CRAS)** e **Região de Planejamento (RP)**.
> ⚠️ O CadÚnico não cobre a totalidade da população municipal.
> As variáveis derivadas dessa fonte representam a população cadastrada ou elegível ao cadastro.
---
...
Capital Humano	0,445	0,258	−0,187	Maior melhora absoluta — dimensão mais dinâmica em Hortolândia
...
Os dados históricos do IPEA fornecem a primeira evidência empírica para sustentar
a redistribuição de pesos proposta no IVS-H:
Dimensão	Variação IPEA (2000→2010)	Hipótese de peso IVS-H	Fundamento empírico
Infraestrutura Urbana	+0,005 (estável)	~15–20% ↓	Confirmado: quase sem variação — baixo poder discriminatório entre territórios de Hortolândia. Reduzir o peso é metodologicamente justificado.
Capital Humano	−0,187 (maior melhora)	~40–45% ↑	Confirmado: foi a dimensão que mais variou. Isso indica que é onde a política pública teve maior impacto — e onde a vulnerabilidade residual é mais relevante para monitorar.
Renda e Trabalho	−0,139 (segunda maior melhora)	~35–40% ↑	Confirmado: segunda dimensão mais dinâmica. A tensão entre polo de emprego e vulnerabilidade persistente justifica peso elevado.
> **Conclusão metodológica:** a distribuição histórica da variação do IVS por dimensão
> é coerente com as hipóteses de calibração do IVS-H. Os pesos propostos não são
> arbitrários — estão ancorados no comportamento real das dimensões em Hortolândia
> ao longo de uma década.
>
> A calibração definitiva será realizada após o cálculo do IVS-H com dados reais
> (CadÚnico dez/2025 + Censo 2022), confrontando os resultados com esta linha de base.
---
...
8	CH_05	% mães chefes de família, sem fund. completo e com filho < 15 anos	CadÚnico ✅	6,25%	~8%	Disponível imediato. Alta relevância local — peso elevado no IVS-H.
9	CH_06	Taxa de analfabetismo — população de 15 anos ou mais	CadÚnico ✅ / IBGE Censo 2022	6,25%	~7%	Disponível imediato. Censo 2022 permite validação para população fora do cadastro.
10	CH_07	% crianças em domicílios sem nenhum morador com fund. completo	CadÚnico ✅	6,25%	~8%	Disponível imediato. Indicador mais poderoso de reprodução intergeracional da pobreza.
11	CH_08	% jovens 15–24: não estudam, não trabalham e renda ≤ ½ SM	CadÚnico + CAGED	6,25%	~6%	Jovens que não estudam nem trabalham ("geração nem-nem") em situação de vulnerabilidade econômica. Jovens fora do CadÚnico são ponto cego.
---
...
✅ Disponível imediato (CadÚnico)	5	CH_05, CH_06, CH_07, RT_01, RT_04	Calcular subíndice IVS-H parcial com CadÚnico dez/2025
⏳ Parcial — curto prazo	9	IU_01, IU_02, CH_01, CH_02, CH_03, CH_08, RT_02, RT_03, RT_05	Integrar IBGE Censo 2022, CAGED e Saúde/Educação municipais
⏳ Parcial — médio prazo	2	IU_03, CH_04	Censo 2022 (mobilidade) + registro de parto adolescente
> ⚠️ A renda domiciliar por setor censitário ainda não foi publicada no Censo 2022.
> Consequentemente, a dimensão **Renda e Trabalho** do IVS-H utiliza como fonte primária
> o CadÚnico municipal, complementado por dados administrativos (CAGED).
> Essa abordagem privilegia a identificação da vulnerabilidade na população atendida
> pelas políticas sociais.
---
Cross-references
Documento	Localização
DIM_VARIAVEL_IVS v01r2 (detalhe técnico completo)	`02_modelagem_lógica/dim_variavel_IVS.md`
schema_IVS.sql (modelagem relacional)	`02_modelagem_lógica/schema_IVS.sql`
DIM_PROGRAMA v11 (campo `dimensao_ivs`)	`02_modelagem_lógica/dim_programas_sociais_v11.md`
Palestra março 2026	`00_governanca/palestra_marco_2026.md`
...
v04	2026-03-13	Seção "Linha de Base Empírica" adicionada: IVS IPEA 2000/2010 para Hortolândia obtido via Google BigQuery (basedosdados); tabela de validação empírica das hipóteses de peso IVS-H por dimensão; conclusão metodológica sobre coerência entre variação histórica e calibração proposta; referência ao arquivo CSV adicionada nos Cross-references
---
