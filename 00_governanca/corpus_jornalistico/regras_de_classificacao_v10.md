...
flowchart TD
    ...
    B{"Ocorre em Hortolândia\nou tem impacto?\nR14"}
    B -- NÃO --> C([DESCARTAR])
    B -- SIM --> D

    subgraph C1 ["CAMADA 1 — Classificação Estrutural"]
        D["tipo_camada"]
        D --> D1[IVS]
        D --> D2[GOVERNANCA]
        D --> D3[PRESSAO_SOCIAL]
        ...
        D1 & D2 & D3 & D4 --> E
        E{"dimensao_analitica\nR16 / R17"}
        E --> E1[capital_humano]
        E --> E2[infraestrutura_urbana]
        E --> E3[renda_trabalho]
        E --> E4["vazio\n(só se puramente narrativo)"]
    end

    E1 & E2 & E3 & E4 --> F

    subgraph C2 ["CAMADA 2 — Qualificação Analítica"]
        F{"Existe mensuração\nobjetiva?\nR01"}
        F -- SIM --> G["tipo_relacao = direta\ncodigo_variavel = código IVS\nou vazio (R04)"]
        F -- NÃO --> H["Aplicar R11\nação do Estado → indireta\nsem ação → contextual"]

        G & H --> I

        I["observacao\nR15 — hierarquia obrigatória"]
        I --> I1["há número verificável\n→ dado_auditavel"]
        I --> I2["sem eixo dominante\n→ sinal_smids_ext"]
        I --> I3["contexto institucional\n→ contexto_sociopolitico"]
    end

    I1 & I2 & I3 --> J

    subgraph C3 ["CAMADA 3 — Integração Dinâmica"]
        J["papel_no_ciclo\nR19"]

        J --> J1{"tipo_camada =\nPRESSAO_SOCIAL?"}
        J1 -- SIM --> J1a["emergencia\nou agravamento\n⛔ NUNCA nao_aplicavel"]

        J --> J2{"tipo_camada =\nGOVERNANCA?"}
        J2 -- "responde pressão" --> J2a[resposta]
        J2 -- "estrutural / sem gatilho" --> J2b[nao_aplicavel]

        J --> J3{"tipo_camada =\nCONTEXTO ou IVS?"}
        J3 -- "integra ciclo?\n(casos raros)" --> J3a["papel adequado"]
        J3 -- NÃO --> J3b[nao_aplicavel]

        J1a & J2a & J2b & J3a & J3b --> K

        K["id_caso_pressao\n(opcional)\nex: CH_VIOLENCIA_GENERO_2025"]

        K --> L{"entra_ipst?"}
        L -- "PRESSAO_SOCIAL\n+ alerta\n+ dado_auditavel" --> L1[sim]
        L -- "atende parcialmente" --> L2[avaliar]
        L -- "sem relevância\npara IPST-H" --> L3[nao]
    end

    L1 & L2 & L3 --> M([REGISTRO COMPLETO])

    style C1 fill:#e8f4f8,stroke:#2980b9
    style C2 fill:#e8f8e8,stroke:#27ae60
    style C3 fill:#fdf3e3,stroke:#e67e22
